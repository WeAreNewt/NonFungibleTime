...
export class NftCollectionService
    ...
    public mint(
        ...
        { userAddress, name, description, category, availabilityFrom, availabilityTo, duration, royaltyBasisPoints }: MintParamsType,
    ): EthereumTransactionTypeExtended[] {

        const collectionContract = this.getContractInstance(this.collectionAddress);
        const txCallback: () => Promise<transactionType> = this.generateTxCallback({
            rawTxMethod: async () =>
                collectionContract.populateTransaction.mint(name, description, category, availabilityFrom, availabilityTo, duration, royaltyBasisPoints),
            from: userAddress,
    ...
    public async buyToken(
        ...
        if (currency === API_ETH_MOCK_ADDRESS) {
            ...
            const { isApproved, approve }: IERC20ServiceInterface =
                this.erc20Service;
    ...
    public redeem(
        ...
        const txCallback: () => Promise<transactionType> = this.generateTxCallback({
            rawTxMethod: async () =>
                collectionContract.populateTransaction.redeem(tokenId),
            from: userAddress,
            value: DEFAULT_NULL_VALUE_ON_TX,
        });
